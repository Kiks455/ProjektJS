...
class Add extends React.Component {
    ...
    async componentDidMount(){
        await recordingsService.getRecordings()
            .then(async recordings => {
                ...
                const r = recordings.map(a => a.id).filter(b => b.length != 36).map(c => parseInt(c)).reduce((a, b) => Math.max(a, b), 0)+1;
                await this.setState({
                    indexa: a,
                    indexr: r
                })
    ...
    async handleSubmit(event) {
        ...
        if (isFormValid) {
            ...
            const recording = new Recording(this.state.indexr.toString(), this.state.title, parseInt(this.state.length), formattedDate, this.state.tags.split(';').map(tag => tag.trim()), artistss, parseInt(this.state.rating), this.state.comments.split(';').map(comment => comment.trim()))
            await this.setState({
                indexr: this.state.indexr+1
            })
            await recordingsService.addNewRecording(recording)
            await this.setState({
                title: '',
                length: '',
                releaseDate: new Date(),
            ...
            <Card bg='info'>
                ...
                <Card.Body>
                    <form onSubmit={this.handleSubmit}>
                        ...
                        <Row className="justify-content-md-center"><Button type="submit" onClick={() => this.handleSubmit}>Add Recording</Button></Row>
                    </form>
                </Card.Body>
            </Card>
            </Container>
